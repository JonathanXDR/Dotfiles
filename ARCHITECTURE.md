...
│   └── run_after_09-*                                   # Symlink /etc/hosts → ~/.config/hosts (every apply, self-healing)
│
...
The `run_onchange_after_04-fix-icloud-permissions` script re-runs whenever the iCloud path, the machine type, or the names and modes of the files under the secrets directory change, and fixes the permissions on the symlink targets (broadly `0700` for directories and `0600` for private material, with SSH public keys and client config plus the work VPN profiles left readable at `0644`).
...
| `run_after_`          | Every `chezmoi apply`, after files  | Export keychain to iCloud, repair /etc/hosts link    |
...
| **Proxy-dependent npm registry**                               | Both hosts behind the `@swisscom` scope front the same virtual repo, but only `artifactory.swisscom.com` is reachable through the corporate proxy and only `bin.swisscom.com` without it. npm has no per-proxy registry setting, so `.npmrc` expands `${NPM_SWISSCOM_REGISTRY}` from the environment and `proxy:set`, `proxy:unset` and `proxy-watchd` swap it as the network changes. The two URLs live once in `.chezmoidata.toml`. `.zshenv` defaults the variable to the direct host, because it is the one shell file every zsh reads, so `zsh -c`, ssh sessions and cron resolve the scope too. `.exports` would have covered interactive shells alone. The functions set rather than unset it, because an ini file has no fallback and npm would otherwise use the variable name literally.                                                                                                                                                                                        |
...
| **`run:daily` update gating**                                  | The `run:daily` wrapper puts `mise:update`, `brew:check` and `plugins:update` behind a stamp file in `~/.cache/daily/`, with a `(N.mh-24ms+0)` freshness check that costs zero forks and ignores a future-dated stamp. Keeps slow update commands off every shell startup.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **`/etc/hosts` symlink**                                       | chezmoi renders `dot_config/hosts.tmpl` into `~/.config/hosts` with machine-type-aware entries (work entries are dropped on personal). Script 09 symlinks `/etc/hosts` → `~/.config/hosts`. It runs on every apply rather than behind a content hash, because the state it depends on is `/etc/hosts` itself, which a macOS upgrade or an installer can replace with a regular file. It backs up the first real file it finds before replacing it.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
